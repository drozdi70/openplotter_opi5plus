...
class pypilotFrame(wx.Frame):
	...
	def onRead(self):
		...
		try:
			subprocess.check_output(['ls', '-ltr']).decode(sys.stdin.encoding)
		except:
			self.ShowStatusBarRED(_('I2C is disabled. Please enable I2C5_M3 interface in Orange Pi 5 configuration /boot/armbianEnv.txt'))
			self.imuDetected.SetLabel(label+_('Failed'))
		else:
			SETTINGS_FILE = "/usr/local/lib/python3.11/dist-packages/pypilot/RTIMULib"
			...
			if os.path.exists(path):
				with open(path, 'r') as f:
					for line in f:
						line = line.replace('\n', '')
						line = line.strip()
						if '/dev/ttyS4' in os.path.realpath(line) : exists = True
			if not exists:
